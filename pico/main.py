...
#_________________________________
SELECT_COL_PINS = (board.GP13, board.GP14, board.GP15)
SELECT_ROW_PINS = (board.GP7, board.GP8)
SELECT_KEYCODES = (
    Keycode.W, Keycode.S, Keycode.T, 
    Keycode.A, Keycode.D, Keycode.G
)
select_keys = keypad.KeyMatrix(SELECT_ROW_PINS, SELECT_COL_PINS)
#_________________________________
...
while True:
    ...
    select_event = select_keys.events.get()
    if special_event:
        special_key_number = select_event.key_number
        if select_event.pressed:
            kbd.press(SELECT_KEYCODES[select_key_number])
        if select_event.released:
            kbd.release(SELECT_KEYCODES[select_key_number])
